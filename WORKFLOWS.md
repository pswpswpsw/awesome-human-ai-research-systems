...
| **(screenshot only)** | the wording exists inside a vendor screenshot rather than as page text. You can see it in the picture, but you cannot select it or search for it, so it is not †. Covidence's inter-rater reliability columns are the example |

A label in a heading, such as (documented), or in bold at the start of a paragraph, is the default for what follows. A sentence carrying its own label in brackets is the exception to that default, and several sections mix the two. Where a sentence rests on a vendor blog post or a vendor evaluation, the label table governs instead of the heading.
...
Every trace below asks whether a program can drive the system with nobody watching, so a few technical words need explaining once. An **API** is a way for one program to use another program's service directly, with no person clicking anything. A script sends a message and gets an answer back. An **endpoint** is a single address inside that API, and each address does one job. Elicit's `POST /api/v2/sessions/systematic-reviews` is an endpoint. The `POST` part is the instruction word, meaning do this thing. The other one you will see is `GET`, which only reads something back. A **schema**, usually published as an **OpenAPI document**, is the rulebook for those messages. It lists every endpoint, every field you may send, and which fields you cannot leave out. The list of fields you cannot leave out is called `required`, and it repays close reading, because a field that is not required is a field a program can simply omit. When a job takes minutes rather than milliseconds, the service often answers straight away and finishes the work later. **Polling** means the script keeps asking whether the job is done. A **webhook** is the opposite arrangement, where the service calls the script back once it finishes, and neither of the two systems here with an API documents one. **MCP** is a recent convention for handing an AI assistant a list of tools it may call, so an assistant rather than a script drives the service. An **SDK** is a ready-made library a vendor ships so you do not have to write the messages by hand. A **404** is a web server saying that the address does not exist.
...
| Question card | no, and the agent decides when to raise one | per question, with a bulk Skip | UI | denied automatically after 10 minutes, and the agent proceeds on its own basis |
...
**What nobody has evaluated.** There is no published evaluation of the dual-review conflict workflow itself. Not conflict rates, not adjudication quality, not whether blinded reviewers anchor on the model's visible recommendation. The recommendation is deliberately shown to both reviewers while their own decisions are hidden from each other, so anchoring is the obvious threat to the mechanism, and it is untested in public.
