...
> **Note:** one entry, which says more about the market than about the boundary. Retrieval that offers no opinion is close to extinct as a product category: the commercial pressure runs toward attaching a score, a label, or an extracted value to everything returned, which moves a product one section up. SciSpace sat here in an earlier draft and was moved for exactly that reason. A tool that returns papers and says nothing about them is now unusual enough to be worth naming.
...
| [agent-literature-review](https://github.com/Arcadia-Science/agent-literature-review) | Last pushed April 2025, API-dependent, with no frozen release to fall back on, so the activity rule has nothing to hold onto. The retrieval-and-discussion loop it offers is also covered in stronger forms by entries above, and what distinguishes it from them is a conversational terminal, which is a front end. |
